...
export const CreateReportModal = ({ navigation }) => {
        ...
        fields: [
          ...
          },
          {
            ...
            text:
              ...
              " " +
              useSelector((state) => state.user.user.LastName),
          },
          {
            type: "plain_text",
            text: displayedValue,
  ...
  const postMessageToChannelWithImage = () => {
    console.log(url),
      axios
        .post(ApiConfig.slackWebHook, messagewithImage)
        .then(function (response) {
          console.log(response);
          closeModal();
          setSubmit(false);
  ...
  const Footer = (props) => (
    <Layout {...props}>
      <Button
        appearance="ghost"
        status="danger"
        onPress={() => cancellReport()}
      >
        Cancel
      </Button>
      <Button
        accessoryLeft={submit ? LoadingIndicator : null}
  ...
  const Header = (props) => (
    <Layout {...props}>
      ...
      <Text category="s1" appearance="hint">
        Issues or suggestions? Please describe it here so our developers can
        find a solution.
      </Text>
    </Layout>
  );
    ...
    <React.Fragment>
            ...
            <ScrollView>
              ...
              >
                <SelectItem
                  ...
                  accessoryLeft={problemIcon}
                />
                <SelectItem
                  title="Suggest an improvement"
                  accessoryLeft={improvementIcon}
                />
                <SelectItem
              ...
              {uploadUrl && (
                <Image
                  source={{ uri: uploadUrl.assets[0].uri }}
      ...
      >
        <Card disabled={true} header={SuccessHeader}>
          <Text status={"success"} style={{ margin: 15, alignSelf: "center" }}>
      ...
      >
        <Card disabled={true} header={UnsuccessHeader}>
          <Text status={"danger"} style={{ margin: 15, alignSelf: "center" }}>
            Something went wrong. Please, try again.
          </Text>
          <Button
            appearance="outline"
            size={"small"}
            onPress={() => toggleUnsuccessNotificationOff()}
            status={"danger"}
          >
            OK
          </Button>
