...
export default class QRScanScreen extends React.Component {
  ...
  async checkIfStudentIsEnrolled(enrollmentId) {
    ...
    if (enrollment !== null && enrollment !== undefined)
      this.setState({ onReadFoundStudent: true, scannedStudent: enrollment });
    else this.setState({ onReadNotFoundStudent: true });
  }

  markStudent() {
    this.props.route.params.checkStudentById(
      this.state.scannedStudent.StudentId
    );
    this.toggleFoundModalOff();
  }

  toggleFoundModalOff() {
    this.setState({ onReadFoundStudent: false, scannedStudent: null });
  }

  toggleNotFoundModalOff() {
    this.setState({ onReadNotFoundStudent: false });
  }
  ...
  render() {
    const foundStudentModal = () =>
      this.state.scannedStudent && (
        <Modal
          visible={this.state.onReadFoundStudent}
        ...
        >
          <Card disabled={true} status="success">
            ...
            <Text style={{ marginBottom: 15, alignSelf: "center" }}>
              {this.state.scannedStudent.StudentName}
            </Text>
            <View style={{ flexDirection: "row", flex: 1, width: 300 }}>
              <View style={{ flexDirection: "column", flex: 1 }}>
                <Button
                  appearance="outline"
                  size={"small"}
                  onPress={() => this.toggleFoundModalOff()}
                  status="basic"
                >
                  Cancel
                </Button>
    ...
    const notFoundStudentModal = () => (
      <Modal
        visible={this.state.onReadNotFoundStudent}
        style={styles.popOverContent}
      >
        <Card disabled={true} status="danger">
          ...
          <Text style={{ marginBottom: 15, alignSelf: "center" }}>
            The student is not enrolled or does not exist.
          </Text>
          <View style={{ flexDirection: "row", flex: 1, width: 300 }}>
            <View style={{ flexDirection: "column", flex: 1 }}>
              <Button
                appearance="outline"
                size={"small"}
                onPress={() => this.toggleNotFoundModalOff()}
                status="danger"
              >
                OK
              </Button>
    ...
    return (
      <Layout style={{ flex: 1 }}>
        ...
        <QRCodeScanner
          ...
          reactivateTimeout={1}
          topContent={
            <Text>Scan student`s QR code for checking his attendance</Text>
          }
        />
      </Layout>
    );
